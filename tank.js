...
export default function main() {
    ...
    const camera = HELPER.makeCamera();
    camera.position.set(8, 4, 10).multiplyScalar(3);
    camera.lookAt(0, 0, 0);
    const targetCamera = HELPER.makeCamera();
    const turretCamera = HELPER.makeCamera();
    const tankCamera = HELPER.makeCamera(75);
    ...
    function render(time) {
        ...
        const selectedCamera = cameras[(time * 0.25) % cameras.length | 0];
